...
** TODO Fly.io Distributed System Challenge with Go :dev:go:@systems:
:PROPERTIES:
...
This part was really about familiarizing oneself with the [[https://github.com/jepsen-io/maelstrom][Maelstrom]] testbench, which the challenge utilizes to abstract node-level operations (~send~, sync/async ~rpc~, etc.).

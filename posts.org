...
In my small project of implementing a [[https://github.com/paul-kang-1/dns-go][minimal DNS lookup utilitly in Go]], I was trying to come up with a simplified logic of parsing the query response into a ~DNSPacket~ struct, which had the following structure:
#+begin_src go
